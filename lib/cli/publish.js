...
program.version(pkg.version)
  ...
  .option('-m, --message', 'A message for the commit to the gh-pages branch')
  .parse(process.argv);
...
build({
  ...
  run('git ls-tree --name-only HEAD | grep -v .gitignore | xargs rm -rf');
  // Copy from ignored dist folder (that survived branch switch) to top level
  run(`cp -R ${ outputDir }/ .`);
  ...
  // Switch back to wherever we came from
