...
export const ExpandJSON = CachingWriter.extend({
  ...
  updateCache(srcPath, destDir) {
    ...
    forOwn(documentType, (fileMetadata, relativepath) => {
      ...
      let data = defaults(this.data(fileMetadata, relativepath), metadata);
      // Allow pages to override their template
      let pageTemplate;
      if (data.page && data.page.layout) {
        let pageTemplatePath = path.join(srcPath, '__theme', data.page.layout + '.hbs');
        pageTemplate = handlebars.compile(fs.readFileSync(pageTemplatePath, 'utf-8'));
      } else {
      ...
      // Write it!
      fs.writeFileSync(destFile, pageTemplate(data));
    });
  }
});
